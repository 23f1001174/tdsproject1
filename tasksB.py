...
def B4(repo_url, commit_message):
    ...
    return f"Repository cloned and commit added: {commit_message}"
...
def B10(csv_path, filter_column, filter_value):
    ...
    import json
    if not B12(csv_path):
        return json.dumps({"error": "Access outside /data is not allowed."})
    ...
    try:
        ...
        return json.dumps({"filtered_data": filtered.to_dict(orient='records')})
